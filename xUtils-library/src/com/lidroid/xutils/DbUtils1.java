...
public class DbUtils1 extends DbUtils{
     ...
     * key: dbName
     */
    private static HashMap<String, DbUtils1> daoMap = new HashMap<String, DbUtils1>();

    private SQLiteDatabase database;
    private boolean debug = false;
    private boolean allowTransaction = false;

    private DbUtils1(DaoConfig config) {
        if (config == null) {
    ...
    private synchronized static DbUtils1 getInstance(DaoConfig daoConfig) {
        DbUtils1 dao = daoMap.get(daoConfig.getDbName());
        if (dao == null) {
            dao = new DbUtils1(daoConfig);
    ...
    public static DbUtils1 create(Context context) {
        DaoConfig config = new DaoConfig(context);
        return getInstance(config);
    }

    public static DbUtils1 create(Context context, String dbName) {
        DaoConfig config = new DaoConfig(context);
        config.setDbName(dbName);
        return getInstance(config);
    }

    public static DbUtils1 create(Context context, String dbDir, String dbName) {
        DaoConfig config = new DaoConfig(context);
    ...
    public static DbUtils1 create(Context context, String dbName, int dbVersion, DbUpgradeListener dbUpgradeListener) {
        DaoConfig config = new DaoConfig(context);
        config.setDbName(dbName);
    ...
    public static DbUtils1 create(Context context, String dbDir, String dbName, int dbVersion, DbUpgradeListener dbUpgradeListener) {
        DaoConfig config = new DaoConfig(context);
        config.setDbDir(dbDir);
        config.setDbName(dbName);
        config.setDbVersion(dbVersion);
    ...
    public static DbUtils1 create(DaoConfig daoConfig) {
        return getInstance(daoConfig);
    }

    public DbUtils1 configDebug(boolean debug) {
        this.debug = debug;
        return this;
    }

    public DbUtils1 configAllowTransaction(boolean allowTransaction) {
    ...
    private boolean saveBindingIdWithoutTransaction(Object entity) throws DbException {
        ...
        if (idColumn.isAutoIncrement()) {
            ...
            if (entityKvList != null && entityKvList.size() > 0) {
                ...
                DbUtils1.fillContentValues(cv, entityKvList);
