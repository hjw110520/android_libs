...
public class DbUtils2 extends DbUtils{
     ...
     * key: dbName
     */
    private static HashMap<String, DbUtils2> daoMap = new HashMap<String, DbUtils2>();
    ...
    private DbUtils2(DaoConfig config) {
        if (config == null) {
        ...
        SQLiteDatabase.loadLibs(config.getContext());
        this.database = createDatabase(config);
        this.daoConfig = config;
    }


    private synchronized static DbUtils2 getInstance(DaoConfig daoConfig) {
        DbUtils2 dao = daoMap.get(daoConfig.getDbName());
        if (dao == null) {
            dao = new DbUtils2(daoConfig);
    ...
    public static DbUtils2 create(Context context) {
        DaoConfig config = new DaoConfig(context);
        return getInstance(config);
    }

    public static DbUtils2 create(Context context, String dbName) {
        DaoConfig config = new DaoConfig(context);
        config.setDbName(dbName);
        return getInstance(config);
    }

    public static DbUtils2 create(Context context, String dbDir, String dbName) {
        DaoConfig config = new DaoConfig(context);
    ...
    public static DbUtils2 create(Context context, String dbName, int dbVersion, DbUpgradeListener dbUpgradeListener) {
        DaoConfig config = new DaoConfig(context);
        config.setDbName(dbName);
    ...
    public static DbUtils2 create(Context context, String dbDir, String dbName, int dbVersion, DbUpgradeListener dbUpgradeListener) {
        DaoConfig config = new DaoConfig(context);
        config.setDbDir(dbDir);
        config.setDbName(dbName);
        config.setDbVersion(dbVersion);
    ...
    public static DbUtils2 create(DaoConfig daoConfig) {
        return getInstance(daoConfig);
    }

    public DbUtils2 configDebug(boolean debug) {
        this.debug = debug;
        return this;
    }

    public DbUtils2 configAllowTransaction(boolean allowTransaction) {
    ...
    private SQLiteDatabase createDatabase(DaoConfig config) {
        ...
        if (!TextUtils.isEmpty(dbDir)) {
            ...
            result = SQLiteDatabase.openOrCreateDatabase(dbFile,"secret_key", null);
        } else {
            result = SQLiteDatabase.openOrCreateDatabase(config.getDbName(), "secret_key", null);
        }
        return result;
    }
    ...
    private boolean saveBindingIdWithoutTransaction(Object entity) throws DbException {
        ...
        if (idColumn.isAutoIncrement()) {
            ...
            if (entityKvList != null && entityKvList.size() > 0) {
                ...
                DbUtils2.fillContentValues(cv, entityKvList);
